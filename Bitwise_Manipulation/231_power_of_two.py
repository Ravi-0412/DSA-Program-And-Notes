...
# method 2: No in power of two has 2's complement equall to the num itself
...
# so '&'  with pre no should always give 0 and

# method 4: 
# for no in power of 2, it has one bit more than the its pre no
# so '&'  with pre no should always give 0 and pre no contain all 1's only at bit posotion where power of two contain all 0's only
