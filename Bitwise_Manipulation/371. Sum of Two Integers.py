...
class Solution {
    public int getSum(int a, int b) {
        int mask = 0xFFFFFFFF; // Mask to get 32-bit integer results

        while (b != 0) {
            int carry = (a & b) & mask; // Calculate carry
            a = (a ^ b) & mask;         // Calculate the sum without carry
            b = (carry << 1) & mask;    // Calculate the carry and shift it left
        }
