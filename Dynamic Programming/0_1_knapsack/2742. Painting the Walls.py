...
# if we use painter 'i'th then remaining walls to paint = walls - time[i -1] - 1.
# Reason: '1' will be painted by the paid painter, and 'time[i-1]' walls will be paid by the free painter during that time.

# Note:
# when we paint a wall by the painter1 then we can skip time[i] walls (basically this time[i] no. of walls will be painted by the painter2 with zero cost).

# So we can re-think the problem as select n walls from the given one so that the sum of the times to pain the wall(painded by painter 2) + 
# the number of wall taken (painted by painter 1) > total no. of walls to paint.

# This is similar to the 0/1 Knapsack where the cost is same as weight and our bag size is wall count. 
# But in Knapsack we reduce the bag size by the stone size here we reduce the wall count by the time[i] and additional -1 to reduced currently painted wall.
    ...
    def paintWalls(self, cost: List[int], time: List[int]) -> int:
        ...
        def solve(painter , walls):
            if walls <= 0:
                ...
                return 0
            if painter == 0:
                # means all walls are not painted and there is no paid painter avaiable
                return float('inf')
