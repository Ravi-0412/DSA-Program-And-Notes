# inution behind Q:
# 1) if we have been given then we could have applied binary search for each number.
# time: O(n*logn)
...
# for minimisng the final ans, we will make incorrect guess for minimum no from the options keeping in mid picking this number will reduce our subproblems to size= 1.

# why MCM type Q?
# Ans: here no guess function is given, so we have to pick any number from '1' to 'n' and go till worst.
...
# Also read comment under above link by "shenhaizhilong".
...
class Solution:
    def getMoneyAmount(self, n: int) -> int:
        ...
        def minAmount(i, j):
            if i>= j:   # suppose there is more than '1' ele and if we choose '1' then i= 1 and j= 0. so include >= instead of "==".
                return 0
            ans= float('inf')
            for k in range(i, j+1):
                tempAns=  k + max(minAmount(i, k-1) , minAmount(k+1, j))  # 'k' is the penalty for choosing incorrectly
                ans= min(ans, tempAns)
            return ans
