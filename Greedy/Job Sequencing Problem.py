...
class Solution {
    public int[] JobScheduling(Job[] Jobs_given, int n) {
        Arrays.sort(Jobs_given, (a, b) -> b.profit - a.profit);

        int maxDeadline = 0;
        for (Job job : Jobs_given) {
            maxDeadline = Math.max(maxDeadline, job.deadline);
        }

        int[] position = new int[maxDeadline + 1];
...
class Solution {
    public int[] JobScheduling(int[][] Jobs, int n) {
        ...
        int[] position = new int[maxDeadline];
        Arrays.fill(position, -1);
