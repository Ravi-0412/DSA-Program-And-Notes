...
class Solution:
    def insertionSortList(self, head: Optional[ListNode]) -> Optional[ListNode]:
        dummy= ListNode(0)
        pre= dummy  # it will point to the last node in soretd list till now. will tell the node from where we have to start checking for correct position. 
        # we will move 'pre' only to find the correct position and after finding the correct position we will insert node between 'pre' and 'pre.next'.
        while head:
            if pre.val > head.val:    # last sorted node agar bda (i.e pre) then hmko start se check karna hoga(dummy.next se). isliye pre= dummy kar do.
