...
class Solution:
    def find132pattern(self, nums: List[int]) -> bool:
        ...
        for j in range(n):
            ...
            if s1 == nums[j]:
                continue
            # check if there exist any nums[k] to right of 'j' following 132 pattern.
...
# Logic: First think how we can track s2 and s3 i.e make sure that s3 < s3 exist.
# 's3' must be the largest number smaller than 's2' on right side of 's2' to get a valid s1 candidate to the left.
