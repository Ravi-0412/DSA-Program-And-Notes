...
# this will be valid for any number of mismatch nodes


# method 2: No need to store the ans in array
...
# second_violation_At: will the curr last node where violation occcured
...
# logic: phla violation ka pre lena h(phla wala node) simce it's first time and dusra vilation ka dusra node lena h since 2nd violation
...
class Solution:
    def recoverTree(self, root: Optional[TreeNode]) -> None:
        ...
        if first_violation_By and second_violation_At:  # means there is only one violation and both the nodes are adjacent
            first_violation_By.val, second_violation_At.val= second_violation_At.val, first_violation_By.val
        elif first_violation_By and first_violation_At: # means there is two violation and both the nodes are not adjacent
            first_violation_By.val, first_violation_At.val= first_violation_At.val, first_violation_By.val


# Note: VVVI for BST problem.. 
# In most of the BST Q,every manipulation and changes is done while we print the node for the inorder traversal.. keep this in mind and 
# try to think what changes you can make there to get the ans
