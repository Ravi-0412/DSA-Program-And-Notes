...
class Solution:
    def bstFromPreorder(self, preorder: List[int]) -> TreeNode:
        def build(preorder):
            ...
            root.left = build(preorder[1:i])   # Left elements are smaller
            root.right = build(preorder[i:])   # Right elements are greater
            return root
...
class Solution {
    ...
    private TreeNode build(int[] preorder, int start, int end) {
        ...
        // Find the index where right subtree starts (greater than root)
        while (i < end && preorder[i] < root.val) {
            i++;
        }

        root.left = build(preorder, start + 1, i);  // Elements smaller than root
        root.right = build(preorder, i, end);       // Elements greater than root
        return root;
    }
}
"""
...
"""
Idea is simple but above one is more simpler and best.
...
2.b) If value is greater than last item in stack, pop it. It will be right child of last item.
...
Q. why we are directly adding when num is smaller and poping before adding when num is greater?
