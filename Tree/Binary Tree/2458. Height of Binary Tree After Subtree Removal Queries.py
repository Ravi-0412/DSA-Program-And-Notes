...
# How to think?
# Because we can get maxHeight after deleting any node from 1) different subtree i.e max height elsewhere in the tree
...
class Solution:
    def treeQueries(self, root: Optional[TreeNode], queries: List[int]) -> List[int]:
        ...
        def dfs(root, depth, maxDepth):
            ...
            dfs(root.left , depth + 1 , max(maxDepth , depth + height(root.right)))   # for 'left' we will find the maxHeight from its sibling i.e 'right' & same for 'right'.
