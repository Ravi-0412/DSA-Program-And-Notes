# Brute force: O(n^3)
# for every node check for every subtree (O(n)) whether they are same or not (O(n))= O(n^3)

# method 2:
# for each node find the serialisation and store that serialisation as key with node.val as "val" in a hashmap.
...
class Solution:
    def findDuplicateSubtrees(self, root: Optional[TreeNode]) -> List[Optional[TreeNode]]:
        subtree= defaultdict(list)  # {serialisation_node: node_values}
        ...
        def dfs(root):
            ...
            if len(subtree[s]) == 1:  # means duplicate subtree
                ans.append(root)
